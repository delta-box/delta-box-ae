#!/usr/bin/env python3
"""Verify restore wakeup experiments and redraw complete-API comparisons."""
import argparse
...
DEFAULT = ROOT / 'ae/report/restore-wakeup-20260921'
...
COMMITS = {'base': 'c33a42f', 'loop': '74a09be', 'opt': 'b228b18'}
REPEATS = {'base': 5, 'loop': 1, 'opt': 5}
LABELS = {'base': 'Before / prewarm on', 'loop': 'Resume loop only',
          'opt': 'Resume loop + prewarm I/O / prewarm on'}
...
def collect(source, output):
    ...
    for run in sorted(source.iterdir()):
        if not run.is_dir() or run.name.endswith('-env'):
            continue
        if not run.name.startswith(('restore-',)):
            continue
...
def scheduler_and_prewarm(archive):
    result = {}
    with tarfile.open(archive) as saved:
        members = {m.name: m for m in saved.getmembers()}
        if 'prewarm.jsonl' in members:
            rows = [json.loads(line) for line in saved.extractfile(members['prewarm.jsonl'])]
            result['prewarm_stats'] = dict(log_n=len(rows), ok_n=sum(r['ok'] for r in rows),
                mean_elapsed_ms=statistics.mean(r['elapsed_ms'] for r in rows),
                mean_pages=statistics.mean(r['pages'] for r in rows),
                mean_skipped=statistics.mean(r.get('skipped', 0) for r in rows))
        if 'restore-diagnostics.json' in members:
            result['diagnostic_setup'] = json.load(saved.extractfile(members['restore-diagnostics.json']))
        if 'restore-kernel-trace.txt' not in members:
            return result
        trace = saved.extractfile(members['restore-kernel-trace.txt']).read().decode()
    counts = re.search(r'entries-in-buffer/entries-written:\s+(\d+)/(\d+)', trace)
    result['trace_buffer_overwritten'] = bool(counts and counts[1] != counts[2])
    windows, current, discarded = [], None, 0
    for line in trace.splitlines():
        parsed = re.search(r'\s(\d+\.\d+): (\w+): (.*)', line)
        if not parsed:
            continue
        timestamp, event, payload = float(parsed[1]), parsed[2], parsed[3]
        if 'tracing_mark_write' == event and payload.startswith('restore_begin '):
            discarded += current is not None
            ids = re.search(r'active=(\d+) template=(\d+)', payload)
            current = dict(start=timestamp, active=int(ids[1]), template=int(ids[2]), events=[]) if ids else None
        if current is None:
            continue
        current['events'].append((timestamp, event, payload))
        if event == 'tracing_mark_write' and payload.startswith('restore_end '):
            windows.append(current)
            current = None
    discarded += current is not None
    samples = []
    for window in windows:
        active, template = window['active'], window['template']
        def first(event, *tokens, after=-math.inf):
            return next((t for t, e, p in window['events'] if t >= after and e == event
                         and all(token in p for token in tokens)), None)
        kill = first('signal_generate', 'sig=9 ', f'pid={active} ')
        exited = first('sched_process_exit', f'pid={active} ')
        continued = first('signal_generate', 'sig=18 ', f'pid={template} ')
        scheduled = first('sched_switch', f'next_pid={template} ', after=continued) if continued is not None else None
        if None in (kill, exited, continued, scheduled):
            discarded += 1
            continue
        samples.append(dict(active=active, template=template,
            kill_signal_to_exit_ms=(exited-kill)*1000,
            continue_to_running_ms=(scheduled-continued)*1000))
    result['scheduler'] = dict(complete_windows=len(samples), discarded_windows=discarded, samples=samples,
        means_ms={k: statistics.mean(r[k] for r in samples) for k in
                  ('kill_signal_to_exit_ms', 'continue_to_running_ms')} if samples else {})
    return result
...
def summarize(output):
    ...
    for name in lock['runs']:
        ...
        record.update(scheduler_and_prewarm(directory / 'diagnostics.tar.gz'))
        phase = defaultdict(list)
        for row in events:
            for key, value in row.get('restore_fast_fork_timing', {}).items():
                phase[key].append(value)
        record['fork_phase_means_ms'] = {k: statistics.mean(v) for k, v in phase.items()}
        result['runs'][name] = record
        prof = profile(directory / 'diagnostics.tar.gz')
        if prof:
            result['profiles'][name] = prof
        if name.startswith('restore-race-'):
            record['lifecycle'] = verify_lifecycle(directory / 'diagnostics.tar.gz')
        match = re.fullmatch(r'restore-control-(base|loop|opt)-\d+', name)
        if not match:
            continue
        group = match[1]
        assert record['commit'].startswith(COMMITS[group])
        assert record['prewarm'] is True
        assert not prof and not any(v == '1' and ('PROFILE' in k or 'DIAGNOSTICS' in k) for k, v in config['guest_env'].items())
        settings = (config['guest_flags'], config['guest_env'])
        benchmark_config = benchmark_config or settings
        assert settings == benchmark_config, name
    ...
    for group, metrics in pooled.items():
        ...
        assert len(runs) == REPEATS[group], (group, len(runs))
        result['groups'][group] = dict(repeats=len(runs), mean_ms={m: statistics.mean(v) for m, v in metrics.items()},
            event_p95_ms={m: sorted(v)[math.ceil(len(v)*.95)-1] for m, v in metrics.items()},
            foreground_measured_sum_ms=statistics.mean(r['foreground_measured_sum_ms'] for r in runs),
    ...
    with (output / 'per-run.csv').open('w') as f:
        w = csv.writer(f, lineterminator='\n'); w.writerow(['run', 'group', 'commit', *METRICS])
...
def plot(output, data):
    ...
    for ax, metric, title, paper in zip(axes[:2], METRICS[:2], ['Checkpoint: complete API', 'Restore: complete API'], [12.12, 2.23]):
        ...
        ax.bar(range(len(groups)), means, color=['#64748b', '#26836b'], alpha=.8)
        for i, g in enumerate(groups):
            ...
            ax.annotate(f'{means[i]:.3f}', (i, max(values)), xytext=(0, 9),
                        textcoords='offset points', ha='center', zorder=5,
    ...
    axes[3].bar(range(len(groups)), [data['groups'][g]['mean_ms'][metric] for g in groups],
                       color=['#64748b', '#26836b'], alpha=.8)
    for i, g in enumerate(groups):
        values = [r['mean_ms'][metric] for r in data['runs'].values() if r.get('group') == g]
        axes[3].scatter([i]*len(values), values, c='black', s=20, zorder=4)
        axes[3].annotate(f"{data['groups'][g]['mean_ms'][metric]:.3f}",
                        (i, max(values)), xytext=(0, 9), textcoords='offset points', ha='center',
                        bbox=dict(facecolor='white', edgecolor='none', pad=1))
    ...
    for ax in axes:
        ax.set_xticks(range(len(groups)), [LABELS[g].replace(' / ', '\n') for g in groups], fontsize=8)
    fig.suptitle('Django-14997 / NUMA 2 / requested maximum P-state / 5 complete traces per group\n'
                 'Black points: independent run means. Prewarm stays ON. Historical async-full profile.', fontsize=11)
