...
def _criu_opts(field):
    try:
        return field.GetOptions().Extensions[opts_pb2.criu]
    except KeyError:
        return None


def _marked_as_hex(field):
    opts = _criu_opts(field)
    return bool(opts and opts.hex)


def _marked_as_ip(field):
    opts = _criu_opts(field)
    return bool(opts and opts.ipadd)


def _marked_as_flags(field):
    opts = _criu_opts(field)
    return opts.flags if opts else ""


def _marked_as_dev(field):
    opts = _criu_opts(field)
    return bool(opts and opts.dev)


def _marked_as_odev(field):
    opts = _criu_opts(field)
    return bool(opts and opts.odev)


def _marked_as_dict(field):
    opts = _criu_opts(field)
    return opts.dict if opts else ""


def _custom_conv(field):
    opts = _criu_opts(field)
    return opts.conv if opts else ""
